		...
		<div class="warp">
		    ...
		    <div class="container">
		        <div class="row">
					...
					<!-- 推荐房屋信息模块(开始) -->
		<div class="warp" v-if="$check_action('/house_information/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/house_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_house_information = !show_list_house_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_house_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 房屋信息推荐列表组件 -->
		                <list_house_information v-if="show_list_house_information" :list="list_house_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_house_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_house_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_house_information" :key="i" @click="to_details('house_information',o,'house_information_id')">
		                            <td v-for="(oj,n) in fields_house_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	import mixin from "@/mixins/page.js";
				import list_house_information from "@/components/diy/list_house_information.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_article: true,
							show_list_house_information: true,
				list_house_information: [],
				vm_arr_house_information: [
		...
		methods: {
			...
			toToggle(){
				this.isAdmin = !this.isAdmin;
						...
						// 获取房屋信息列表
			get_house_information() {
				let url = "~/api/house_information/get_list?";
										this.$get(url, {
					...
					if (json.result) {
						this.list_house_information = json.result.list;
					}
				})
			},
		...
		computed: {
					    list_table_house_type() {
		        var list = this.list_house_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_house_information.map((o) => {
		                // 第二个中括号是对象的属性
