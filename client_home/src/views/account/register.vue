								...
								<b-form>
									...
									<b-form-group id="input-group-7" label="身份:" label-for="input-7">
										<b-form-select id="user_group" v-model="form.user_group"
											...
											<!--承租用户-->
									<form_rental_user v-if="form.user_group === '出租用户'" :form="form_sub"></form_rental_user>
									<form_tenant_user v-if="form.user_group === '承租用户'" :form="form_sub"></form_tenant_user>
...
<script>
	import mixin from "@/mixins/page.js";
			/* 出租用户 */
	import form_rental_user from "@/components/diy/form_rental_user";
			/* 承租用户 */
	...
	export default {
		...
		methods: {
			 ...
			 */
			get_list_user_group() {
				this.$get("~/api/user_group/get_list?register_max=3&register_min=1", {}, (res) => {
					if (res.result.list) {
						var lt = [];
