<template>
	<div class="diy_details_box diy_div_house_information">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
					...
					<div class="details_btn_wrap">
											<button class="btn btn-primary details_btn" v-if="$check_action('/housing_application/edit','add')" @click="to_form('/housing_application/edit')" > 申请看房 </button>
										<button class="btn btn-primary details_btn" v-if="$check_action('/message_message/edit','add')" @click="to_form('/message_message/edit')" > 留言 </button>
										<button class="btn btn-primary details_btn" v-if="$check_action('/lease_application/edit','add')" @click="to_form('/lease_application/edit')" > 申请租赁 </button>
											<button class="btn btn-primary details_btn" v-if="$check_action('/house_information/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/house_information/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/house_information/set?house_information_id=' + _this.obj["house_information_id"], {
								praise_len
							}, (res) => {
								if(res.error){
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var house_information_id = this.obj.house_information_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "house_information",
          source_field: "house_information_id",
          source_id: house_information_id
        }, (res) => {
		...
		created() {
													...
													this.user_group = this.$store.state.user.user_group;
